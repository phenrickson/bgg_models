...
Another way of looking at what the model learned is to see its predictions on the training set. The models are trained on games published before `r user_results$end_train_year`; of these games, what did the model like for the user?
...
older_games %>%
        make_top_n_table(
                ...
                outcome = user_results$outcome,
                years = seq(user_results$end_train_year -10,
                            user_results$end_train_year),
                top_n = 10
        ) %>%
        gt::tab_header(
                title = paste("Top Games by Year for",
                              get_username()),
                subtitle = paste("Rankings based on predictive model trained on user's collection using games released through", user_results$end_train_year)
        ) %>%
        set_gt_theme() %>%
        set_gt_tab_options() %>%
...
wflows = user_results$training_predictions %>%
        pull(wflow_id) %>%
        unique()
...
But how well does my modeling approach do in predicting *new* games? To test this, I assessed the performance of the model (which was trained on games published through `r user_results$end_train_year`) on games published in `r paste(user_results$end_train_year  +1, user_results$end_train_year + user_results$valid_window, sep = "-")`.
...
newer_games = 
        bind_rows(
                user_results$valid_predictions,
                user_results$upcoming_predictions %>%
                        filter(yearpublished > user_results$end_train_year + user_results$valid_window)) %>%
        filter(wflow_id == 'all_trees_lightgbm')
...
explain_plots = map(ids,
                    ~ get_game_data(game_data = user_results$user_collection %>%
                                            join_bgg_games(games),
                                    id = .x) %>%
                            lightgbm_interpret(game_data = .,
                                               outcome = user_results$outcome) %>%
                            lightgbm_interpret_plot(outcome = user_results$outcome,
                                                    n_features = 15)+
                            theme(plot.title = element_text(hjust = 0,
                                                            size = 8,
                                                            vjust = 2),
