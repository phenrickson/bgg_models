...
make_effects_plot = function(estimates,
        ...
        estimates %>%
                ...
                # labs(subtitle = 
                #              str_wrap("Coefficients from a penalized (ridge) logistic regression for games owned by specified user. Predictors centered and scaled."))
        

                # mutate(link = paste0('https://boardgamegeek.com/boardgame',
                #                      category,
                #                      '/',
                #                      id),
                #      #  link= glue::glue("[{value}]({link})"),
                #        feature = map2(value,
                #                       link,
                #                       ~ gt_hyperlink(.x, .y))) %>%
                # mutate(type,
...
In addition to training a logistic regression, I trained another type of model using boosted trees (LightGBM) which tends to be 

Features that are important in predicting a user's collection will appear towards the top of cover, frequency, and/or gain.
...
```{r separation plots, fig.height = 8}
...
How well calibrated are the models?
...
newer_games %>%
        ...
        gt::tab_header(
                title = paste("Top", n, "(Newer) Games for",
                              get_username()),
                subtitle = paste("Rankings based on predictive model trained on user's collection using games released through",
                                 user_output$end_train_year)
        ) %>%
...
```{r shap, warning =F, message =F, fig.height = 8}
...
Finally, I combine the model's estimates from training and upcoming games into one table. 

```{r new and upcoming games datatable, warning = F, message=F}

bind_rows(older_games,
          newer_games) %>%
