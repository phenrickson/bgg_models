...
predict_hurdle = function(data, model, threshold, ...) {
    data |>
        add_hurdle(...) |>
        augment(new_data = _, model) |>
        select(-.pred_class, -.pred_no) |>
        add_pred_class(threshold = threshold) |>
        rename(.pred_hurdle_yes = .pred_yes, .pred_hurdle_class = .pred_class)
}
...
finalize_model = function(wflow, data, ratings = 25, weights = 5) {
    outcome =
        wflow |>
...
pin_outcome_model = function(wflow, board, tuning, metrics, data, ...) {
...
pin_model = function(model, model_name, board, metadata, check_renv = T, ...) {
    model |>
...
pin_read_model = function(board, metadata) {
...
prepare_outcome_split = function(
    data,
    outcome,
    weights,
    ratings,
    ...
    settings = tibble(weights = weights, ratings = ratings)

    tibble(outcome = outcome, settings = list(settings), split = list(split))
}

prepare_outcome_recipe = function(split, outcome, recipe, ...) {
    training =
        split |>
        pluck("split", 1) |>
        training()
...
prepare_outcome_wflow = function(prepared, model_spec, wflow_id = NULL, ...) {
    rec =
    ...
    prepared |>
        select(-recipe) |>
        add_column(wflow = list(wflow), wflow_id = wflow_id) |>
        select(outcome, wflow_id, everything())
}

tune_outcome_wflow = function(
    prepared,
    grid,
    metrics = my_reg_metrics(),
    control = control_grid()
...
finalize_outcome_wflow = function(tuned, metric = 'rmse') {
    result =
        tuned |>
        pluck("result", 1)
    ...
    tuned |>
        select(-wflow) |>
        add_column(params = list(best_params), wflow = list(wflow))
}

fit_outcome_wflow = function(tuned, data = 'all') {
    wflow =
        tuned |>
        pluck("wflow", 1)
    ...
    if (data == "all") {
        ...
        dat = bind_rows(split |> training(), split |> validation())
    } else {
        fit =
            wflow |>
            fit(data)
    ...
    val_id =
        ...
        dplyr::filter(
            yearpublished > end_train_year &
                yearpublished <= end_train_year + valid_years
        ) |>
        pull(.row_number)
...
outcome_tuning_split = function(
    ...
    data |>
        ...
        create_year_split(
            end_train_year = max(data$yearpublished, na.rm = T) -
...
spline_vars = function(vars = c("number_mechanics", "number_categories")) {
    vars
}

# function to predict and calculate bayesaverage given average and usersrated
predict_bayesaverage = function(
    data,
    average_model,
    usersrated_model,
    ratings = 2000
    ...
    data |>
        ...
        select(yearpublished, game_id, name, starts_with(".pred"), everything())
}

# function to tune model given workflow and resamples
tune_workflow = function(
    workflow,
    resamples,
    metrics,
    grid,
...
recipe_linear = function(data, outcome, ids, predictors, splines) {
    data |>
        build_recipe(
            outcome = {{ outcome }},
            ids = ids,
...
build_outcome_recipe =
    function(data, outcome, ids, predictors, splines) {
        data |>
            build_recipe(
                outcome = {{ outcome }},
                ids = ids,
...
# basic recipe setup
build_recipe = function(data, outcome, ids, predictors, ...) {
    recipe(x = data) %>%
        # set ids
        update_role(
            any_of(ids),
...
add_dummies = function(recipe, variable, threshold = 100) {
...
add_bgg_dummies = function(
    ...
    recipe %>%
        # include most mechanics
        add_dummies(mechanics, threshold = mechanics_threshold) %>%
        # include all categories
        add_dummies(categories, threshold = categories_threshold) %>%
        # families
        add_dummies(families, threshold = families_threshold) %>%
        # publishers
        add_dummies(publishers, threshold = publishers_threshold) %>%
        # designers
        add_dummies(designers, threshold = designers_threshold) %>%
        # artists
        add_dummies(artists, threshold = artists_threshold) %>%
        # components
        add_dummies(components, threshold = components_threshold) %>%
        # themes
        add_dummies(themes, threshold = themes_threshold) %>%
        # mechanisms
        add_dummies(mechanisms, threshold = mechanisms_threshold)
}
...
add_preprocessing = function(recipe) {
    recipe %>%
        ...
        update_role(image, thumbnail, new_role = "id") %>%
        ...
        # number_mechanics
        step_mutate(
            number_mechanics = dplyr::case_when(
                is.na(mechanics) ~ 0,
        ...
        # number categories
        step_mutate(
            number_categories = dplyr::case_when(
                is.na(categories) ~ 0,
                TRUE ~ stringr::str_count(categories, ',') + 1
            )
        ) %>%
        # log time per player and playingtime
        step_log(time_per_player, playingtime, offset = 1) %>%
        ...
        step_mutate(
            published_before_1900 = dplyr::case_when(
        ...
        step_mutate(
            deluxe_edition = dplyr::case_when(
                grepl("kickstarter|big box|deluxe|mega box", tolower(name)) ==
                    T ~
                    1,
                TRUE ~ 0
            )
        ...
        step_mutate(
            description_from_publisher = dplyr::case_when(
                grepl("description from publisher", tolower(description)) == T ~
                    1,
                TRUE ~ 0
            )
        )
...
add_imputation = function(recipe) {
    recipe %>%
        ...
        step_mutate(
            minplayers = dplyr::case_when(
                minplayers > 10 ~ 10,
                TRUE ~ minplayers
            )
        ) %>%
        step_mutate(
            maxplayers = dplyr::case_when(
                maxplayers > 20 ~ 10,
                maxplayers <= 1 ~ 1,
                TRUE ~ maxplayers
            )
...
# add corr
add_corr = function(recipe, my_threshold = 0.9) {
    recipe %>%
        step_corr(all_numeric_predictors(), threshold = my_threshold)
}
...
add_discrete = function(
    recipe,
    outcome,
    vars = c("year", "number_mechanics", "number_categories")
) {
    outcome = enquo(outcome)

    # embed categorical feature via mixed
    step_discretize_feature = function(recipe, feature, outcome) {
    ...
    for (i in 1:length(vars)) {
        recipe = recipe %>%
            step_discretize_feature(feature = !!vars[i], outcome = !!outcome)
        # add_role(!!vars[i],
        #          new_role = "discrete")
    }
...
add_splines = function(
    recipe,
    vars = c("year", "number_mechanics", "number_categories"),
    degree = 5
) {
    step_spline_feature = function(recipe, feature, ...) {
        feature = enquo(feature)
...
predict_sim = function(fit, new_data, nsims = 1) {
    ...
    simulateX(mod, nsim = nsims, X = prepped) %>%
        tibble()
}

# define training and valid split
make_train_valid_split = function(data, end_train_year, valid_window = 2) {
    ...
    valid =
        ...
        filter(
            yearpublished > end_train_year &
                yearpublished <= end_train_year + valid_window
        )
...
make_resamples = function(data, outcome, seed = 1999) {
    missing = data %>%
    ...
    if (length(missing) > 0) {
        stop(rlang::englue(paste(
            length(missing),
            "missing observations in {{outcome}}"
        )))
    }

    # otherwise, create folds
    resamples = vfold_cv(data, v = 5, strata = {{ outcome }})

    message(rlang::englue(paste(
        "creating training and resamples for {{outcome}} with",
        nrow(data),
        "obs"
    )))
...
create_workflows = function(data, recipes, models) {
    # create workflow set
    workflow_set(
        preproc = recipes,
        models = models,
...
control_grid = function(save_pred = TRUE, parallel_over = "resamples") {
    control_resamples(
        save_pred = save_pred,
        save_workflow = T,
        allow_par = T,
...
preprocess_outcome = function(data, outcome, ratings = 25, weights = 5) {
...
impute_averageweight = function(data, model) {
    model %>%
...
truncate_averageweight = function(x) {
    case_when(x > 5 ~ 5, x < 1 ~ 1, TRUE ~ x)
}

# sim averageweight
sim_averageweight = function(data, fit, sims = 1) {
    sims = fit %>%
        predict_sim(new_data = data, nsims = sims) %>%
        mutate(across(starts_with("sim_"), truncate_averageweight)) %>%
        bind_cols(., data)
...
round_usersrated = function(log_usersrated, round = 50) {
...
calculate_bayesaverage = function(data, ratings = 2000) {
    data %>%
        mutate(
            # numerator
            .pred_bayesaverage = ((ratings * 5.5) +
                (.pred_average * .pred_usersrated)) /
                # denominator
...
predict_average = function(data, model) {
    model %>%
        augment(data) %>%
        rename(.pred_average = .pred)
}

predict_usersrated = function(data, model, round = 50) {
    model %>%
        augment(data) %>%
...
simulate_outcomes = function(
    ...
    simulate_averageweight = function(...) {
        simulateX(averageweight_mod, nsim = sims, X = prepped) %>%
        ...
        foreach(i = 1:ncol(sims_averageweight), .combine = 'bind_rows') %do%
            {
                ...
                tibble(
                    .pred_average = simulateX(
                        average_mod,
                        nsim = 1,
                        X = prepped_outcome
                    ) %>%
                        pull(),
                    .pred_usersrated = simulateX(
                        usersrated_mod,
                        nsim = 1,
                        X = prepped_outcome
                    ) %>%
